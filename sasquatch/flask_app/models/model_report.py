...
#statiic method for checking if report is valid to register
    @staticmethod
    def validator(data:dict) -> bool:
        is_valid = True
        if len(data['location']) < 1:
            flash("location must be at least 1 characters.", "err_reports_location")
            is_valid = False

        
        if len(data['description']) < 1:
            flash("Description must be at least 3 characters.", "err_reports_description")
            is_valid = False
    
        if len(data['date']) < 3:
            flash("Date must be at least 3 characters.", "err_reports_date")
            is_valid = False
        
        if len(data['num_of']) < 1:
            flash("Number of must be at least 1 characters.", "err_reports_num_of")
            is_valid = False
